...
def ask_assistant(message: str, assistant_id: str = None):
    ...
    logger.info(conversation_messages.data[0].content[0].text.value)
...
def update_assistant_tools_with_actions():
    ...
    if current_assistant is None:
        logger.error("Failed to retrieve current assistant.")
        return None
...
def interactive_session(assistant_id: str = None):
    ...
    logger.info(_colored("Welcome to the interactive session. Type your message and press enter to get a response. Type 'quit' to end the session.", 'yellow'))
    
    while True:
        message = input(_colored("You: ", 'green'))
        if not message.strip():
            logger.info(_colored("Message cannot be empty. Please enter a message.", 'red'))
            continue
        if message.lower() == "quit":
            logger.info(_colored("Thank you for using the interactive session. Goodbye!", 'yellow'))
            break
        try:
            conversation_manager.add_message(message)
            conversation_messages = conversation_manager.process_conversation()
            logger.info(_colored("Assistant: ", 'blue'), conversation_messages.data[0].content[0].text.value)
        except Exception as e:
            logger.info(_colored(f"An error occurred: {e}", 'red'))
...
def set_docs_as_knowledge():
    ...
    logger.info("updating assistant with knowledge in the docs folder")
