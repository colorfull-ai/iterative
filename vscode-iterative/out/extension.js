...
exports.IterativeProjectManager = exports.deactivate = exports.activate = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
function findIterativeProjects(workspaceFolders) {
    const projects = [];
...
async function updateIconThemeForProjects(projects) {
...
async function setIterativeIconTheme() {
    ...
    try {
        ...
        if (currentTheme !== 'iterative-icons') {
            ...
            const action = await vscode.window.showInformationMessage('Icon theme updated. Reload window to apply changes?', 'Reload Window');
...
async function checkAndSetIconTheme() {
    ...
    try {
        ...
        if (projects.length > 0) {
            console.log(`Found ${projects.length} iterative projects:`, projects.map(p => `${p.parentName} (${p.parentPath})`));
...
function activate(context) {
    ...
    let convertToIterative = vscode.commands.registerCommand('iterative.convertToIterative', async (uri) => {
        if (!uri) {
    ...
    let createApiFile = vscode.commands.registerCommand('iterative.createApiFile', async (uri) => {
        if (!uri) {
...
`;
    ...
    let createModelFile = vscode.commands.registerCommand('iterative.createModelFile', async (uri) => {
        if (!uri) {
...
`;
    ...
    context.subscriptions.push(createProject, openManager, manageProject, convertToIterative, createApiFile, createModelFile, focusMode, iterativeWatcher, apiWatcher, serviceWatcher, servicesWatcher, modelsWatcher, disposable);
}
...
class IterativeProjectManager {
    ...
    async _checkFolder(projectPath, folderType) {
        try {
    ...
    async _startServer(projectPath) {
        try {
    ...
    async _openInEditor(projectPath) {
        try {
    ...
    async _updateSetting(settingName, value) {
        try {
            switch (settingName) {
